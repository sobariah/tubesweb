...
Class Action {
	...
	function signup(){
		function pictureValidation(): bool {
			$type = $_FILES['img']['type'];
			$pattern = "/^image\/.*/";
			return (bool) preg_match($pattern, $type);
		}

		if (pictureValidation() == false) {
			return 2;
		}
		...
		if($save){
			...
			if($_FILES['img']['tmp_name']){
							...
							$data .= ", avatar = '$fname' ";
							// $fileext=pathinfo($_FILES, PATHINFO_EXTENSION);


			}

			function image_upload(){
				$data['img']="upload image";
				$this->db->query('signup', $data);
			}
			function ajax_upload(){
				if (isset($_FILES['img']['tmp_name'])) {
					...
					if (!this->upload->do_upload('img')) {
						...
						echo '<img src=" '.base_ur().'assets/uploads/.'.$data["file"].'"  />';
					}
